...
__device__ int* GetUniqueExercisePrice(float* allData, int begin, int end, float cpType, float *uniqueExercisePrice, float *nowData)
{
...
__device__ void Combination(int *arr, int *data, int start, int end, int index, int r, int *ans) 
{ 
...
__device__ int ComputeArbitrage(float* uniqueExercisePrice, float* nowData, int* size)
{
    int ans = 0;
    if(size[0] >= 3)
    {
...
__global__ void worker(float *allData, float *timeIdx, int timeCount, int *callAns, int *putAns)
{
    int i = blockIdx.x*MAXTHREADSIZE + threadIdx.x;
    if(i < timeCount-1)
    {
        int begin;
        ...
        callAns[i] = ComputeArbitrage(uniqueCallExercisePrice, nowCallData, a);
        putAns[i] = ComputeArbitrage(uniquePutExercisePrice, nowPutData, b);
...
int main()
{
    ...
    while((dirp = readdir(dp)) != NULL)
    {
        ...
        { 
            ...
            int *callAns;
            int *putAns;
            callAns = new int[timeCount]();
            putAns = new int[timeCount]();
            int *cudaCallAns;
            int *cudaPutAns;
            float *cudaAllData;
            float *cudaTimeIdx;
            cudaMalloc(&cudaCallAns, sizeof(int)*(timeCount));
            cudaMemcpy(cudaCallAns, callAns, sizeof(int)*(timeCount), cudaMemcpyHostToDevice);
            cudaMalloc(&cudaPutAns, sizeof(int)*(timeCount));
            cudaMemcpy(cudaPutAns, putAns, sizeof(int)*(timeCount), cudaMemcpyHostToDevice);
            cudaMalloc(&cudaAllData, sizeof(float)*(MAXROWCOUNT*5));
            cudaMemcpy(cudaAllData, allData, sizeof(float)*(MAXROWCOUNT*5), cudaMemcpyHostToDevice);
            cudaMalloc(&cudaTimeIdx, sizeof(float)*(MAXROWCOUNT));
            cudaMemcpy(cudaTimeIdx, timeIdx, sizeof(float)*(MAXROWCOUNT*5), cudaMemcpyHostToDevice);
            int blockSize = ceil((float)timeCount/MAXTHREADSIZE);
            worker<<<blockSize, MAXTHREADSIZE>>>(cudaAllData, cudaTimeIdx, timeCount, cudaCallAns, cudaPutAns);
            cudaMemcpy(callAns, cudaCallAns, sizeof(int)*(timeCount), cudaMemcpyDeviceToHost);
            cudaMemcpy(putAns, cudaPutAns, sizeof(int)*(timeCount), cudaMemcpyDeviceToHost);
            putCount = 0;
            callCount = 0;
            ...
            cudaFree(cudaCallAns);
            cudaFree(cudaPutAns);
            cudaFree(cudaAllData);
            cudaFree(cudaTimeIdx);
