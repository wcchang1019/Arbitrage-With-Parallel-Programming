...
int ComputeArbitrage(vector<string> uniqueExercisePrice, vector<vector<string>> nowData, int begin)
{
    int ans = 0;
            ...
            for(int a=0; a<dataIdx1.size(); a++)
            {
                for(int b=0; b<dataIdx2.size(); b++)
                {
                    for(int c=0; c<dataIdx3.size(); c++)
                    {
                        ...
                        if(tmp1 > stoi(nowData[dataIdx2[b]][4]) || tmp2 > stoi(nowData[dataIdx3[c]][4]) || tmp3 > stoi(nowData[dataIdx1[a]][4]))
                        {
                            //cout << "HAHA" << endl;
                            continue;
                        }
                        //cout << tmp1*stof(nowData[dataIdx2[b]][3]) << endl;
                        if(tmp1*stof(nowData[dataIdx2[b]][3])+tmp2*stof(nowData[dataIdx3[c]][3])-tmp3*stof(nowData[dataIdx1[a]][3]) >= 20*(tmp1+tmp2+tmp3))
                        {
                            //cout << begin << ":";
                            //cout << exercisePrice[2] << "-" << exercisePrice[1] << " " << stof(nowData[dataIdx2[b]][3]) << " " << exercisePrice[2] << "-" << exercisePrice[1] << " " << stof(nowData[dataIdx1[a]][3]) << " " << exercisePrice[1] << "-" << exercisePrice[0] << " " << stof(nowData[dataIdx3[c]][3]) << endl;
                            ans++;
                        }
                    }
                }
...
int main()
{
    ...
    vector<int> timeIdx;
    ReadCsv("2016_01_04.csv", allData, timeIdx);
    int begin = 0;
    ...
    while(timeIdx.size()>0)
    {
        ...
        //ComputeArbitrage(uniquePutExercisePrice, nowPutData);
        cout << ComputeArbitrage(uniquePutExercisePrice, nowPutData, begin) << endl;
        begin = end+1;
    }
    return 0;
}